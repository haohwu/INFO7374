...
def find_similar_images(query_emb, image_data, topn=5):
    # Convert embeddings to numpy arrays
    query_emb = np.array(query_emb)
    image_data['embedding'] = image_data['embedding'].apply(np.array)
    
    # Calculate cosine similarity
    image_data['similarity'] = image_data['embedding'].apply(lambda x: np.dot(x, query_emb) / (np.linalg.norm(x) * np.linalg.norm(query_emb)))
    
    # Get top N similar images
    similar_images = image_data.sort_values(by='similarity', ascending=False).head(topn)
